...
export default function Login() {
  ...
  const onM3u = async (e) => {
    e.preventDefault();
    if (!m3uUrl) {
      toast.error("Ingresa una URL M3U");
      return;
    }
    setLoading(true);
    try {
      const { data } = await api.post("/m3u/parse", { url: m3uUrl });
      login({ mode: "m3u", m3uUrl, parsed: data });
      toast.success(`Cargado: ${data.total} canales`);
      navigate("/", { replace: true });
    } catch (err) {
      toast.error(err?.response?.data?.detail || "No se pudo cargar la lista");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-grid-dim p-8 relative overflow-hidden">
      ...
      <div className="w-full max-w-2xl relative z-10">
        ...
        <div className="bg-[#0a0a0a] border border-neutral-900 rounded-2xl p-10 shadow-2xl">
          <div className="flex gap-2 mb-8 bg-[#111] rounded-xl p-1.5" role="tablist">
            <button
              role="tab"
              data-testid="tab-xtream"
              onClick={() => setTab("xtream")}
              className={`focus-tv flex-1 py-3 rounded-lg font-semibold text-lg transition-colors outline-none ${
                tab === "xtream" ? "bg-[#FFB800] text-black" : "text-neutral-400"
              }`}
            >
              Xtream Codes
            </button>
            <button
              ...
              onClick={() => setTab("m3u")}
              className={`focus-tv flex-1 py-3 rounded-lg font-semibold text-lg transition-colors outline-none ${
                tab === "m3u" ? "bg-[#FFB800] text-black" : "text-neutral-400"
              }`}
            >
              Lista M3U
            </button>
          ...
          {tab === "xtream" ? (
            <form onSubmit={onXtream} className="space-y-5" data-testid="xtream-form">
              <Field label="URL del Servidor" placeholder="http://tuservidor.com:8080" value={server} onChange={setServer} testid="input-server" />
              <Field label="Usuario" placeholder="usuario" value={username} onChange={setUsername} testid="input-username" />
              <Field label="Contraseña" placeholder="••••••••" type="password" value={password} onChange={setPassword} testid="input-password" />
              <button
                type="submit"
                disabled={loading}
            ...
            <form onSubmit={onM3u} className="space-y-5" data-testid="m3u-form">
              <Field label="URL de la lista M3U" placeholder="https://.../playlist.m3u" value={m3uUrl} onChange={setM3uUrl} testid="input-m3u" />
              <button
                type="submit"
                disabled={loading}
                data-testid="m3u-submit"
                className="focus-tv w-full mt-4 bg-[#FFB800] text-black font-bold rounded-xl py-5 text-xl hover:bg-[#FFD147] transition-colors outline-none disabled:opacity-50 flex items-center justify-center gap-3"
              >
                {loading && <Loader2 className="w-5 h-5 animate-spin" />}
                Cargar Lista
